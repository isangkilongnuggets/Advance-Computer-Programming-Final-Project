...
def insert(categ_combobox, categ_amount, category):
    ...
    try:
        ...
        showinfo('Activity Recorded', f'Total Carbon Emission: {total_carbon_emission} gCO₂')
    except:
        return None
...
def double_click(click):
    ...
    def update_quantity_main():
        try:
            ...
            if category == 'Transportation':
                ...
                showinfo('Done', f'The Record\'s quantity was successfully updated to {new_quantity} (Total carbon emission was also updated).')
    ...
    if selected:
        ...
        cancel_button_again = tk.Button(button_frame, text = 'Cancel', bg = '#F97362', fg = 'white', command = lambda: update_dashboard_treeview.pack_forget())
...
def retrieveall_useracts():
    ...
    hide_button = tk.Button(dashboard_treeview, text = 'Hide Logs', command = lambda: hide_logs(), bg = '#F97362', fg = 'white')
    ...
    pfdbcur.execute('SELECT category, category_id, quantity, carbon_emission, activity_timestamp FROM User_Activity WHERE user_id = %s', (loggedon_userid,))
...
def introduction():
    ...
    tk.Label(intro, text = 'PerFoot is a personal footprint tracker that helps you monitor '
                        'and reduce your environmental impact by estimating the carbon footprint '
...
# show the logs of the current user
...
# 'Use_Activity' table treeview and delete all button frame
...
cancel_button = tk.Button(dashboard, text = 'Cancel', command = lambda: (trans_info.pack_forget(), food_info.pack_forget(), house_info.pack_forget(), category.pack_forget(), cancel_button.pack_forget()), bg = '#F97362', fg = 'white')
